...
def _get_material_randomization():
    from .material_randomization import get_material_randomization_class
...
def spawn_door(
    ...
    **kwargs,
    ...
    add_mass(stage, axle_prim_path, mass=0.3)
    ...
    hinge_joint.GetUpperLimitAttr().Set(120)
    hinge_drive = UsdPhysics.DriveAPI.Apply(hinge_joint.GetPrim(), "angular")
    hinge_drive.GetTargetPositionAttr().Set(0.0)
    ...
    hinge_drive.GetDampingAttr().Set(3.0)
    ...
    if cfg.build_latch:
        ...
        set_prim_transform(
            ...
            (-0.083, (half_door_width - 0.005) * door_open_lr, door_handle_height),
            (0, 0, 0),
            (1.0, 1.0, 1.0),
        )
        ...
        latch_joint.CreateLocalPos0Attr().Set(
            Gf.Vec3f(-0.083, (half_door_width - 0.005) * door_open_lr, door_handle_height)
        )
        if door_open_lr == 1:
